...
private def usage : String := String.intercalate "\n" [
  ...
  "",
  "rename",
  row "rename f m (d r)..." "uses, in one file",
  row "rename-file f (d r)..." "uses, in one file, m taken from the path",
  row "rename --glob g (d r)..." "uses, across files [--no-index]",
  row "rename-decl --glob g (d r)..." "uses and binding sites, across files",
  row "rename-module m r" "a module, its file, and every import of it",
  row "rename-token f old new" "a notation token, in one file",
  row "rename-token --glob g old new" "a notation token, across files",
  row "infix --glob g d token" "give d infix notation",
  "",
  "move",
